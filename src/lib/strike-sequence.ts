...
/**
 * Le pas maximum de la frappe, plus petit que sa plus courte fenetre
 * (`fireUp`, 0,05 s) : en dessous, aucun temps fort ne peut etre saute.
 */
export const STRIKE_MAX_STEP = ENVELOPE_MAX_STEP;

/**
 * Avance l'horloge de la frappe d'une image, en bornant le pas.
 *
 * La regle et sa mesure vivent desormais dans `lib/envelope-clock` : elle est
 * nee ici, sur la frappe, mais elle vaut pour tout geste narratif du projet.
 * Cette fonction reste, parce que le nom parle a l'endroit ou elle est
 * appelee, mais elle ne fait plus que deleguer.
 */
export function advanceStrike(since: number, delta: number, maxStep = STRIKE_MAX_STEP): number {
  return advanceEnvelope(since, delta, maxStep);
}

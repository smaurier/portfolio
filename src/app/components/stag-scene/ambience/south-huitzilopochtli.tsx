/* eslint-disable react-hooks/immutability -- pattern gamedev r3f useFrame : mutation des uniforms partages a 60 fps (meme precedent que arrow-vapor). */
...
import { createSouthAmbienceNodeMaterial, type SouthUniforms } from "../webgpu/ambience-tsl";
...
function createSouthMaterial(geometry: BufferGeometry, uniforms: SouthUniforms) {
  if (isWebGpu()) return createSouthAmbienceNodeMaterial(geometry, uniforms);
...
export default function SouthHuitzilopochtli({ alphaRef }: { alphaRef: MutableRefObject<number> }) {
  ...
  const { geometry, uniforms } = useMemo(() => {
    ...
    const geo = createParticleGeometry({ position: { array: positions, itemSize: 3 }, aBird: { array: birds, itemSize: 1 }, aTrail: { array: trailPos, itemSize: 1 } });
    const u: SouthUniforms = {
      uAlpha: { value: 0 },
      uTime: { value: 0 },
  ...
  const material = useMemo(() => createSouthMaterial(geometry, uniforms), [geometry, uniforms]);

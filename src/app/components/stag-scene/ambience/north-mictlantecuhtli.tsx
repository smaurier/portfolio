...
import { createNorthAmbienceNodeMaterial, type NorthUniforms } from "../webgpu/ambience-tsl";
...
function createNorthMaterial(geometry: BufferGeometry, uniforms: NorthUniforms) {
  if (isWebGpu()) return createNorthAmbienceNodeMaterial(geometry, uniforms);
...
export default function NorthMictlantecuhtli({ alphaRef }: { alphaRef: MutableRefObject<number> }) {
  ...
  const { geometry, uniforms } = useMemo(() => {
    ...
    const geo = createParticleGeometry({ position: { array: positions, itemSize: 3 }, aSeed: { array: seeds, itemSize: 1 }, aLifespan: { array: lifespans, itemSize: 1 }, aKind: { array: kinds, itemSize: 1 } });
    const u: NorthUniforms = {
      uAlpha: { value: 0 },
      uTime: { value: 0 },
  ...
  const material = useMemo(() => createNorthMaterial(geometry, uniforms), [geometry, uniforms]);
  const object = useMemo(() => createParticleObject(geometry, material), [geometry, material]);


  useFrame((state) => {
    uniforms.uAlpha.value = alphaRef.current;
    uniforms.uTime.value = state.clock.elapsedTime * NORTH_TIME_SCALE;
    // Texture chargee par useTexture (suspend) : injectee ici, jamais
    // pendant le render (react-hooks/immutability).
    if (uniforms.uSmokeTex.value !== smokeTexture) {
      uniforms.uSmokeTex.value = smokeTexture;
    }
  });
